...
router.post('/', async (req, res) => {
    ...
    console.log(req.body, 'fdfefedfefefergefegrr')
    try {
        ...
        if (cart) {
            const productIndex = cart.products
                .findIndex((p) => p.productId.toString() === productId && p.size === size && p.color === color)
            if (productIndex > -1) {
            ...
            const newCart = await Cart.create({
                ...
                products: [
                    {
                        productId, name: product.name, image: product.images[0].url, price: product.price,
                        size, color, quantity
                    }
                ],
...
router.put('/', async (req, res) => {
    console.log(req.body, 'mognogodbobjfwfffrfrrf')
    const { productId, quantity, size, color, guestId, userId } = req.body
...
router.delete('/', async (req, res) => {
    ...
    console.log(req.body, 'grwrereerrecleared')
    try {
        let cart = await getCart(userId, guestId)
        ...
        if (productIndex > -1) {
            ...
            return res.status(404).json({ message: 'product not found in cart2' })
        }
    } catch (error) {
        console.error(error)
        return res.status(500).json({ message: 'server error' })
    ...
    try {
        ...
        if (guestCart) {
            ...
            if (userCart) {
                ...
                await userCart.save()
                try {
                    await Cart.finrOneAndDelete({ guestId })
                } catch (error) {
