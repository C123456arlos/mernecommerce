...
router.put('/:id', protect, admin, async (req, res) => {
    try {
        ...
        if (product) {
            ...
            product.isFeatured = isFeatured !== undefined ? isFeatured : product.isFeatured || product.isFeatured
            product.isPublished = isPublished !== undefined ? isPublished : product.isPublished || product.isPublished
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (product) {
            ...
            ers.status(404).json({ message: 'product not found' })
        }
    } catch (error) {
        console.log(error)
        res.status(500).send('server error')
