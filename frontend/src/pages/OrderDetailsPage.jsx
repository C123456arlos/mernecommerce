...
const OrderDetailsPage = () => {
    ...
    console.log(orderDetails, 'gadgadgsagagagrrre')
    useEffect(() => {
                ...
                (<div className='p-4 sm:p-6 rounded-lg border'>
                    <div className='flex flex-col sm:flex-row justify-between mb-8'>
                        ...
                        <div className='flex flex-col items-start sm:items-end mt-4 sm:mt-0'>
                            ...
                            <span className={`${orderDetails.isDelivered ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'} px-3 py-1 rounded-full text-sm font-medium mb-2`}>{orderDetails.isDelivered ? 'delivered pending' : 'pending'}</span>
                        </div>
                    </div>
                                ...
                                {orderDetails.orderItems.map((item) => (
                                    <tr key={item.productId} className='border-b'>
                                        <td className='py-2 px-4 flex items-center'>
                                            <img src={item.image} alt={item.name} className='w-12 h-1/2 object-cover rounded-lg mr-4'></img>
