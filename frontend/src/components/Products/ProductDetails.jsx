                ...
                <div className='max-w-6xl mx-auto bg-white p-8 rounded-lg'>
                    <div className='flex flex-col md:flex-row'>
                        <div className='hidden md:flex flex-col space-y-4 mr-6'>
                            {selectedProduct.images.map((images, index) => (<img
                                className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage === images.url ? 'border-black' : 'border-gray-300'}`} key={index}
                                onClick={() => setMainImage(images.url)}
                                src={images.url} alt={images.altText || `thumbnail ${index}`}></img>))}
                        </div>
                        <div className="md:w-1/2">
                            <div className="mb-4">
                                <img src={mainImage} alt="main product"
                                    className="w-full h-auto object-cover rounded"></img>
                            </div>
                        </div>
                        <div className="md:hidden flex overscorll-x-scroll space-x-4 mb-4">
                            {selectedProduct.images.map((images, index) => (<img
                                className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage === images.url ? 'border-black' : 'border-gray-300'}`}
                                onClick={() => setMainImage(images.url)} key={index}
                                src={images.url} alt={images.altText || `thumbnail ${index}`}></img>))}
                        </div>
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex gap-2 mt-2">
                                    {selectedProduct.colors.map((color) => (
                                        <button key={color} className={`w-8 h-8 rounded-full border ${selectedColor === color ? 'border-4 border-black' : 'border-gray-300'}`} onClick={() => setSelectedColor(color)}
                                ...
                                <div className="flex gap-2 mt-2">
                                    {selectedProduct.sizes.map((size) => (
                                        <button key={size}
                                            ...
                                            className={`px-4 py-2 rounded borde ${selectedSize === size ? 'bg-black text-white' : ''}`}>{size}</button>
                                    ))}
                                </div>
                            </div>
                            <div className="mb-6">
